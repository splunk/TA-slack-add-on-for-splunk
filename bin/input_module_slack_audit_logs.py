...
def validate_access_token(helper, access_token, INPUT_STANZA_NAME):
    ...
    try:
        helper.log_info("{} - Validation request sent to URL : {}".format(INPUT_STANZA_NAME, validate_access_token_url))
        header = {
            'Content-type': 'application/json', 
            'Authorization': 'Bearer ' + access_token
        }
        response = json.loads(requests.get(validate_access_token_url, headers=header, proxies=get_proxy(helper)).text)
        helper.log_info("{} - Response : {}".format(INPUT_STANZA_NAME, response))
        if response["ok"]:
            helper.log_info("{} - Access Token Valid".format(INPUT_STANZA_NAME))
        else:
            raise requests.exceptions.RequestException
    except Exception as e:
        if response["ok"]:
            helper.log_error("{} - Response received : {}".format(INPUT_STANZA_NAME, response["ok"]))
        if response["error"]:
            helper.log_error("{} - Response error : {}".format(INPUT_STANZA_NAME, response["error"]))
        helper.log_error(requests.exceptions.RequestException("{} - Request Exception".format(INPUT_STANZA_NAME))) 
...
def handle_audit_logs(helper, auditlogs, epoch_time, max_auditDateTime, check_point_key, ew, INPUT_STANZA_NAME):
    if 'entries' in auditlogs:
        helper.log_info("{} - Count of auditlogs received : {}".format(INPUT_STANZA_NAME, len(auditlogs["entries"])))
        for entry in auditlogs["entries"]:
            this_auditDateTime = entry.get('date_create')
            if(this_auditDateTime <= epoch_time):
                helper.log_info("{} - Events returned during this call were already ingested. So skipping ...".format(INPUT_STANZA_NAME))
...
def get_logs(helper, ew, epoch_time, opt_access_token, INPUT_STANZA_NAME, check_point_key):
    ...
    r = make_request(helper, base_url, epoch_time, opt_access_token, INPUT_STANZA_NAME)
    ...
    try:
        ...
        # Ingest Audit logs
        max_auditDateTime = handle_audit_logs(helper, auditlogs, epoch_time, max_auditDateTime, check_point_key, ew, INPUT_STANZA_NAME)
        # Handling Pagination
...
def collect_events(helper, ew):
    ...
    helper.log_info("{} - Slack Audit addon start execution".format(INPUT_STANZA_NAME))
    ...
    get_logs(helper, ew, epoch_time, opt_access_token, INPUT_STANZA_NAME, check_point_key)
    
    helper.log_info("{} - Slack Audit addon completed execution".format(INPUT_STANZA_NAME))
